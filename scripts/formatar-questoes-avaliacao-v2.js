...
const arquivoDestino = path.join(__dirname, '../MRS/Questoes_Av_Final_FORMATADO_V2.md');
...
function formatarQuestao(numero, conteudo) {
  ...
  for (let linha of linhas) {
    ...
    // Detectar resposta correta
      ...
      respostaCorreta = match ? match[0].toUpperCase() : '';
    }
    // Detectar feedbacks
    else if (linha.match(/Feedback.*[a-d]/i) || linha.match(/Alternativa.*[a-d]/i)) {
      estado = 'feedbacks';
  ...
  alternativas.forEach(alt => {
    ...
    questaoFormatada += `${alternativaLimpa}\n\n`;
  });
  ...
  feedbacks.forEach(feedback => {
    ...
    questaoFormatada += `${feedbackLimpo}\n\n`;
  });
